...
namespace DoanCN
{
    public partial class NhapKho : Form
    {
        ...
        private void NhapKho_Load(object sender, EventArgs e)
        {
            ...
            if (MAXK.maxk == null)
            {
                DataTable dt = db.ExcuteQuery("select top 1(MaHD) from HOADON where MaHD like 'NK%' order by MaHD desc");
              string  ma = int.Parse(dt.Rows[0][0].ToString().Substring(2)) + 1 + "";
                ma = "NK" + ma.PadLeft(4, '0');
                txtmahd.Text = ma.ToString();
                MAXK.maxk = ma;
            }
            else
            {
                DataTable dt = db.ExcuteQuery("select top 1(MaHD),TenKH,TongTien,NVVC from HOADON H,NHANVIEN N where H.MaNV=N.MaNV and MaHD like 'NK%' order by MaHD desc");
        ...
        private void btthemsanpham_Click(object sender, EventArgs e)
        {
            ...
            Chonsanphamnhap csp = new Chonsanphamnhap();
            this.Hide();
            csp.ShowDialog();
        ...
        {
            DataTable dt = db.ExcuteQuery("select top 1(MaHD),TenKH,HoTen,NgayXuat,TongTien,NVVC from HOADON H,NHANVIEN N where H.MaNV=N.MaNV and MaHD like 'NK%' order by MaHD desc");
